...
####
#Creates the config/buildInfo.js file that will be used by buildInfo resource
####
...
# Get git branch
####
BRANCH='master' # `git branch --no-color 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/\1/'`
...
BUILD_INFO_FILE=config/buildInfo.js
